...
void AimLineItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    qreal pulse = (std::sin(timePhase * M_PI * 2.0) + 1.0) * 0.5;

    QColor glowColor = ThemeManager::instance().getPrimaryColor();
    glowColor.setAlpha(80 + int(pulse * 60));
    ...
    painter->setPen(QPen(coreColor, 2.5, Qt::DashLine, Qt::RoundCap, Qt::RoundJoin));
    for (int i = 0; i < m_points.size() - 1; ++i) {
        painter->drawLine(m_points[i], m_points[i + 1]);
    }
    ...
    painter->rotate(timePhase * 360.0);
