...
BallItem::BallItem(BallColor color, BallType type, BallColor secColor, bool locked, qreal radius, int freezeLevel, bool isMystery, bool isKey, BallType containedSkill, QGraphicsItem* parent)
        : QGraphicsObject(parent), m_color(color), m_type(type), m_secColor(secColor), m_isLocked(locked),
          m_radius(radius), m_freezeLevel(freezeLevel), m_isMystery(isMystery), m_isKey(isKey), m_containedSkill(containedSkill) {
}
...
void BallItem::updateData(BallColor color, BallType type, BallColor secColor, bool locked, int freezeLevel, bool isMystery, bool isKey, BallType containedSkill) {
    m_color = color;
    m_type = type;
    m_secColor = secColor;
    m_isLocked = locked;
...
void BallItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    Q_UNUSED(option);
    Q_UNUSED(widget);
    paintBall(painter, QPointF(0, 0), m_radius, m_color, m_type, m_secColor, m_isLocked, m_freezeLevel, m_isMystery, m_isKey, m_containedSkill);
}

void BallItem::paintBall(QPainter* painter, const QPointF& center, qreal radius, BallColor color, BallType type, BallColor secColor, bool locked, int freezeLevel, bool isMystery, bool isKey, BallType containedSkill) {
    painter->save();
    ...
    // =========================================================================
    // ⚡ 1. HYPER-CHARGED PHOTON LASER BALL
    // =========================================================================
    ...
    // =========================================================================
    // 💣 2. APOCALYPTIC THERMO-NUCLEAR BOMB
    // =========================================================================
    ...
    // =========================================================================
    // ۶. لایه‌های موانع تعاملی (یخ، قفل، کلید، مه، محموله)
    // =========================================================================
    if (freezeLevel > 0) {
        painter->save();
        ...
        if (freezeLevel == 1) {
            painter->drawLine(-radius * 0.2, radius * 0.5, 0, 0);
            painter->drawLine(0, 0, radius * 0.6, -radius * 0.3);
        }
        painter->restore();
    }
